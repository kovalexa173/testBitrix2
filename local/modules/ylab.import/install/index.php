...
class ylab_import extends CModule
{
    ...
    public function DoInstall()
    {
		...
		if($request["step"] < 2){
            ...
            $this->installDB();
			$APPLICATION->IncludeAdminFile(Loc::getMessage('YLAB_IMPORT_INSTALL_TITLE'), $this->GetPath()."/install/step1.php");
            
			
		}
        elseif($request["step"] == 2){
            
            COption::SetOptionString($this->MODULE_ID, "text", $request["text"]);
            COption::SetOptionString($this->MODULE_ID, "textarea", $request["textarea"]);
			$APPLICATION->IncludeAdminFile(Loc::getMessage('YLAB_IMPORT_INSTALL_TITLE'), $this->GetPath()."/install/step2.php");
			
		}
        elseif($request["step"] == 3){
            if($request["checkbox"] == "on"){
                COption::SetOptionString($this->MODULE_ID, "checkbox", "Y");
            }else{
                COption::SetOptionString($this->MODULE_ID, "checkbox", NULL);
            }
            
            COption::SetOptionString($this->MODULE_ID, "password", $request["password"]);
			$APPLICATION->IncludeAdminFile(Loc::getMessage('YLAB_IMPORT_INSTALL_TITLE'), $this->GetPath()."/install/step3.php");
        ...
        elseif($request["step"] == 4)
        {
            ...
            $APPLICATION->IncludeAdminFile(Loc::getMessage('YLAB_IMPORT_INSTALL_TITLE'), $this->GetPath()."/install/step4.php");
